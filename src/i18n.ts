...
//
// 两边靠 setLang 保持同步：用户在这里切换，C# 侧 AppConfig.Language 跟着变并落盘，
// 下次启动 getState 把它带回来。
//
// ⚠️ 结构与 WPE x64 的 WebUI/src/i18n.ts <b>逐条对应</b>（回退链、繁体的例外、
// initLang / setLang 的分工）。改这里之前先看一眼那边，别让两个产品长出两套规矩。
 ...
 * 取文案。
 *
 * 键写错时返回键名本身而不是空串 —— 界面上会明晃晃地露出 "home.foo"，
 * 比静默显示空白容易发现得多。
 *
 ...
 * 取文案并填占位符。
 *
 * `tf('log.count', 12)` → 「共 12 条」。按 {0} {1} … 的顺序替换，
 * 不做成模板字符串是为了让五份译文里那些占位符<b>肉眼可查</b>。
 */
export function tf(key: Key, ...args: Array<string | number>): string {
  let s = t(key)
 ...
 * 落盘那一步万一失败也只是「这次没记住」，不该让界面卡在旧语言上。
 */
