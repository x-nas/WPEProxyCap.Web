...
/*
  ...
  【与 WPE x64 同一套】实现逐条对应 WebUI/src/stores/theme.ts：三态用两个字段拼
  （ThemeMode ＋ IsDark），属性写在 <html> 上，浅色不是反相而是同一套设计换一组令牌值。

  【为什么是两个字段而不是一个枚举】IsDark 是「<b>解析后的实际主题</b>」——
  跟随系统时存的是那一刻系统给出的值。C# 侧拿它去刷窗体底色（无边框窗口四周那圈
  缩放内边距露的就是它），那边不需要知道有第三档存在。
  ThemeMode 只回答「这个值是不是跟着系统走出来的」。

  ⚠️ <b>属性写在 <html> 上，不是 .win 上。</b>弹窗、下拉、提示浮层都是 Teleport 到
  body 的，挂在 .win 上它们拿不到令牌，会出现「页面浅色、下拉还是深色」。
*/
...
/*
  ...
  同一个真源（C# 的 AppConfig）、同一条桥（setAppearance 的稀疏报文）、
  同一个入口（标题栏齿轮）。单开一个 store 只会多一处要对齐的地方。

  【为什么不用 localStorage】偏好的真源是 config.json：它要落盘、要跟着用户走。
*/
 ...
 * 系统<b>此刻</b>是不是深色。只读。
 *
 * 与 effective 不是一回事：那个是「现在实际生效的主题」，而这个不管用户选了哪一档，
 * 说的都是系统那边的事。设置弹窗里「跟随系统」那一档的提示要的是后者 ——
...
/* matchMedia 只建一次：每次进设置页都新建一个的话，监听会越挂越多。 */
...
/** 开 / 关游走亮带。先改本地（同步、界面立刻变），再推 C#。 */
export async function setScan(on: boolean): Promise<void> {
  // 无条件同步一次：ref 与 <html> 上的属性万一对不上，点当前这一项永远修不回来
  applyScan()
 ...
 * 系统怎么变都不该动他的选择。
 * 除了改自己的界面，还要把新解析出来的值推回 C#：窗体底色读的是那一份。
 */
 ...
 *
 * @param mode   用户选的那一档。认不出来一律深色（这套皮肤照深色设计）
 * @param isDark 上次解析出来的实际值。<b>只在「跟随系统」而 matchMedia 又不可用时</b>才派上用场
 * @param scan   游走亮带开不开。认不出来当开着
 */
export function initTheme(
  ...
  /*
    「跟随系统」时，C# 手里的 isDark 是<b>上次</b>解析出来的值 ——
    程序关着的时候系统换了深浅，这里解析出来的就和它对不上。
    那一份不只是存档：无边框窗口四周那圈内边距露的是窗体底色，它按 isDark 取色，
    不对齐的话页面是浅的、四周却是一道黑框。只在确实对不上时送一次，且只送 isDark。
  */
...
/**
 * 切主题。先改本地再推 C#，与 setLang 同一条路数。
 */
export async function setTheme(next: Theme): Promise<void> {
  /*
    ⚠️ 属性<b>无条件</b>同步一次，再判要不要往下走。
    先判等再 apply 的话，一旦 ref 与 <html> 上的属性对不上，
    点当前这一项永远修不回来 —— 而那正是用户会去点的那一下。
  */
  applyDocumentTheme()
  ...
  try {
    // mode 与解析后的 isDark 一起送：前者是用户的选择，后者是 C# 侧刷窗体底色要的值
